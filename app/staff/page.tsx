...
export default async function StaffPage() {
  ...
  return <div className="staff-console">
    <SiteNav />
    <main>
      <header className="staff-ops-band"><div className="staff-ops-band-inner"><div><p className="eyebrow"><span /> Studio operations</p><h1>{isManager ? "Manager" : "Trainer"} console</h1><p className="staff-ops-email">{user.email}</p><p>{new Intl.DateTimeFormat("en-US", { weekday: "long", month: "long", day: "numeric" }).format(today)}</p></div><div className="staff-capacity-stat">{classes.length ? <><strong>{bookedPercent}%</strong><span>booked today</span><small>{totalBooked}/{totalCapacity} total spots</small></> : <><strong>No classes scheduled today</strong><span>The studio register is clear.</span></>}</div></div></header>
      <div className="staff-console-content">
        {isManager ? <>
          <div className="animate-fade-up"><StudioPulse stats={pulseStats} teachingLoad={teachingLoad} /></div>
          ...
          <div className="animate-fade-up" style={{ animationDelay: "120ms" }}><RequestsInbox initialRequests={pendingRequests} /></div>
          <div className="staff-lower-grid animate-fade-up" style={{ animationDelay: "180ms" }}><AtRiskMembers members={atRiskMembers} totalCount={atRiskMembersTotal} /><ActivityLog entries={activityEntries} /></div>
        </> : <div className="animate-fade-up"><MyRequests requests={myRequests} /></div>}
        <div className="staff-lower-grid animate-fade-up" style={{ animationDelay: isManager ? "240ms" : "60ms" }}><MemberSearch /><StaffFitBotTiles /></div>
      </div>
    </main>
  </div>;
}
