...
import { TimeOffRequestsCard } from "@/app/components/time-off-requests-card";
...
export function ChatCard({ card, onSelectOption, canBookClass = true }: { card: RichCard; onSelectOption?: (message: string) => void; canBookClass?: boolean }) {
  switch (card.kind) {
    ...
    case "disambiguation": return <section className="chat-card chat-disambiguation-card" aria-label={card.prompt}><h2>{card.prompt}</h2><div>{card.options.map((option) => <button type="button" key={option.sendMessage} onClick={() => onSelectOption?.(option.sendMessage)}><strong>{option.label}</strong>{option.detail ? <small>{option.detail}</small> : null}</button>)}</div></section>;
    case "notice": { const badgeTone = card.tone === "error" ? "badge-danger" : card.tone === "tip" ? "badge-success" : "badge-neutral"; return <section className={`chat-card chat-notice-card chat-notice-${card.tone}`} aria-label={card.title ?? "Notice"}><span className={`badge ${badgeTone}`}>{card.tone}</span>{card.title ? <h2>{card.title}</h2> : null}<p>{card.body}</p></section>; }
    default: { const _exhaustive: never = card; void _exhaustive; return null; }
  }
}
