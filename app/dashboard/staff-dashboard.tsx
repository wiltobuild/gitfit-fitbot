...
export function StaffDashboard({ classes, bookingRateTrend, instructorName }: { classes: StudioClass[]; bookingRateTrend: BookingRateTrend; instructorName: string | null }) {
  ...
  const renderClass = (classRow: StudioClass) => { const level = fillLevel(classRow.booked_count, classRow.capacity); const isPriority = classRow.id === priorityClass?.id; return <li className={`staff-class-row staff-fill-${level}${isPriority ? " staff-class-priority" : ""}`}><div className="staff-class-summary"><strong>{classRow.name}</strong><span>{formatDate(classRow.class_date, { weekday: "short", month: "short", day: "numeric" })} · {formatTime(classRow.start_time)}</span></div><div className="staff-fill-unit"><div className="staff-fill-label"><span className={`badge ${badgeClass(classRow)}`}>{level === "full" ? "Full" : level === "filling" ? "Filling" : "Healthy"}</span><strong>{classRow.booked_count}/{classRow.capacity}</strong></div><span className="staff-fill-track" aria-label={`${classRow.booked_count} of ${classRow.capacity} spots booked`}><span style={{ width: `${Math.min(100, classRow.capacity ? (classRow.booked_count / classRow.capacity) * 100 : 0)}%` }} /></span></div></li>; };
