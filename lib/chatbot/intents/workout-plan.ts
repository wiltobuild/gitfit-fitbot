...
const pool = (names: string[]): Exercise[] =>
  names.map((name, index) => ({
    name,
    durationMinutes: (index % 3) + 3,
    levels:
      index === 5
        ? ["advanced"]
        : index === 4
...
const exerciseLibrary: Record<Goal, Record<Equipment, Exercise[]>> = {
  ...
  flexibility: {
    ...
    dumbbells: pool([
      "Light dumbbell shoulder halos",
      "Goblet squat pry",
      "Dumbbell weighted hip hinge",
      "Supported dumbbell calf stretch",
      "Dumbbell thoracic rotations",
      "Dumbbell windmills"
    ]),
    "full-gym": pool([
      "Cable chest opener stretch",
      "Assisted pull-up lat stretch",
      "Foam roller thoracic extensions",
      "Smith machine ankle mobility",
      "Cable hip flexor mobilization",
      "Hanging shoulder decompression"
    ])
  },
  general: {
    bodyweight: pool([
      "Sit-to-stand squats",
      "Knee or standard push-ups",
      "Bird dogs",
      "Lateral lunges",
      "Plank shoulder taps",
      "Bear crawl"
...
const warmupExercises: Record<
  "cardio" | "flexibility" | "general",
  Exercise[]
> = {
  cardio: pool(["Easy march", "Dynamic leg swings"]),
  flexibility: pool(["Joint circles", "Gentle spinal mobility"]),
  general: pool([
    "Easy march and arm circles",
    "Dynamic hip and shoulder mobility"
  ])
};
const cooldownExercises: Record<
  "cardio" | "flexibility" | "general",
  Exercise[]
> = {
  cardio: pool([
    "Easy walk and nasal breathing",
    "Standing calf and quad stretch"
  ]),
  flexibility: pool([
    "Long exhales in child's pose",
    "Gentle full-body mobility flow"
  ]),
  general: pool([
    "Easy breathing and walk in place",
...
function selectExercises(goal: Goal, equipment: Equipment, level: Level) {
  const exercises =
    exerciseLibrary[goal][equipment] ?? exerciseLibrary[goal].bodyweight;
...
export function generateWorkoutPlan({
  ...
  const warmupMinutes = Math.min(
    5,
    Math.max(3, Math.round(durationMinutes * 0.1))
  );
  ...
  const blocks: WorkoutBlock[] = [
    {
      label: "Warmup",
      exercises: fillBlock(warmupExercises[variant], warmupMinutes)
    },
    {
      label: "Main",
      exercises: fillBlock(
        selectExercises(goal, equipment, level),
        Math.max(1, durationMinutes - warmupMinutes - cooldownMinutes)
      )
    },
    {
      label: "Cooldown",
      exercises: fillBlock(cooldownExercises[variant], cooldownMinutes)
    }
  ];
  return {
    reply: [
      `${weekly ? "Start this week with" : "Here’s"} your ${durationMinutes}-minute ${goal} workout (${equipment}, ${level} level).`,
      ...blocks.map(formatBlock),
      weekly
        ? "Use this as your first session this week, then check back for the next focused session."
    ...
    card: {
      kind: "workout",
      title: `${durationMinutes}-minute ${goal} workout`,
...
export const workoutPlanIntent: Intent = {
  ...
  match: (message) =>
    scoreTriggerFamily(message, [
      /\b(?:help me )?plan my week\b/i,
      /\b(workout|exercise|training)\b/i
    ]) *
    (1 +
      scoreEntity(message, [
        /\b(?:plan my week|plan|build|give me|need (?:a|an)|quick|help me|create|\d{1,3}\s*-?\s*(?:min(?:ute)?s?))\b/i
      ])),
  handle: (message, _session, pendingAnswer) => {
    void pendingAnswer;
    return generateWorkoutPlan({
      durationMinutes: parseDuration(message),
      goal: parseGoal(message),
      equipment: parseEquipment(message),
      level: parseLevel(message),
      weekly: /\b(?:help me )?plan my week\b/i.test(message)
    });
  }
};
