...
function label(row: ClassRow) { const [hours, minutes] = row.start_time.split(":").map(Number); return `${row.name} â€” ${row.class_date}, ${hours % 12 || 12}:${String(minutes).padStart(2, "0")} ${hours >= 12 ? "PM" : "AM"}`; }
...
export const whoIsBookedIntent: Intent = {
  ...
  match: (message) => {
    ...
    const hasClassReference = Boolean(
      resolveDate(normalized) || resolveTime(normalized) || ["yoga", "cycling", "hiit", "boxing", "pilates", "strength", "sofia", "martinez", "marcus", "lee", "avery", "thompson", "diego", "reyes", "elena", "cruz", "jordan", "blake"].some((term) => hasWord(normalized, term)),
    );
  ...
  handle: async (message) => {
    ...
    const date = resolveDate(normalized); const instructor = ["sofia", "martinez", "marcus", "lee", "avery", "thompson", "diego", "reyes", "elena", "cruz", "jordan", "blake"].find((term) => hasWord(normalized, term)); const type = ["yoga", "cycling", "hiit", "boxing", "pilates", "strength"].find((term) => hasWord(normalized, term)); const time = resolveTime(normalized);
    ...
    if (error) return { reply: "I couldnâ€™t retrieve class bookings right now. Please try again shortly." };
    if (classes.length === 0) return { reply: "I couldnâ€™t find a class matching that request." };
    ...
    return { reply: `${label(classRow)} has ${classRow.booked_count} of ${classRow.capacity} spots booked. Member names arenâ€™t available in the current staff view yet.` };
  },
};
