...
export const recommendClassIntent: Intent = {
  ...
  handle: async (_message, session, pendingAnswer) => {
    ...
    type ClassType = NonNullable<ReturnType<typeof resolveClassType>>;
    const resolvedTypes: Array<ClassType | undefined> = (member.preferred_class_types ?? "").split(";").map((value: string) => resolveClassType(value.trim()));
    const preferredTypes = [...new Set(resolvedTypes.filter((type): type is ClassType => Boolean(type)))];
    ...
    const { data: classes, error } = await supabase.from("classes").select("name, type, instructor, class_date, start_time, capacity, booked_count").gte("class_date", todayDate()).in("type", classTypes).order("class_date").order("start_time").limit(5);
