...
export const recommendClassIntent: Intent = {
  ...
  handle: async (_message, session, pendingAnswer) => {
    ...
    if (!preferredTypes.length) {
      // TODO(Phase 7): render as a notice card once the kind exists
      return { reply: "I don't see any class preferences on your profile yet — want me to show the full schedule instead?" };
    }
    ...
    if (!classes?.length) {
      // TODO(Phase 7): render as a notice card once the kind exists
      return { reply: "I don't see any upcoming classes matching what you're into right now — want me to show the full schedule instead?" };
    }
