...
export const memberLookupIntent: Intent = {
  ...
  handle: async (message, _session) => {
    ...
    let bookingsReply = "This member does not have an account yet, so no booking history is available.";
    if (member.auth_user_id) {
      const { data: bookingData, error: bookingError } = await supabase.from("bookings").select("classes!inner(name, type, instructor, class_date, start_time)").eq("user_id", member.auth_user_id).gte("classes.class_date", todayDate()).order("created_at", { ascending: true });
      if (bookingError) { console.error("Unable to retrieve member bookings", bookingError); return { reply: "I found the member, but couldn't retrieve their upcoming bookings right now. Please try again shortly." }; }
      classes = (bookingData ?? []).map((booking) => booking.classes as unknown as ClassRow | null).filter((classRow): classRow is ClassRow => classRow !== null).sort((a, b) => `${a.class_date} ${a.start_time}`.localeCompare(`${b.class_date} ${b.start_time}`));
      bookingsReply = classes.length === 0 ? "No upcoming bookings." : `Upcoming bookings:\n${classes.map((classRow) => `${classRow.name} (${classRow.type}) with ${classRow.instructor} — ${formatDate(classRow.class_date)}, ${formatTime(classRow.start_time)}.`).join("\n")}`;
    }
    return { reply: `${member.full_name || "no name on file"}\n${member.email}\nType: ${member.is_staff ? "staff" : "member"}\nMember since: ${formatDate(member.created_at)}\n${bookingsReply}`, card: { kind: "members", members: [{ name: member.full_name || member.email, email: member.email, status: `${member.is_staff ? "staff" : "member"} · ${classes.length} upcoming booking${classes.length === 1 ? "" : "s"}` }] } };
  },
};
