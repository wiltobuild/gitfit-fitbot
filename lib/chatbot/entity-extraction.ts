...
function levenshteinDistance(left: string, right: string) {
  ...
  for (let leftIndex = 1; leftIndex <= left.length; leftIndex++) {
    ...
    for (let rightIndex = 1; rightIndex <= right.length; rightIndex++) current[rightIndex] = Math.min(current[rightIndex - 1] + 1, previous[rightIndex] + 1, previous[rightIndex - 1] + Number(left[leftIndex - 1] !== right[rightIndex - 1]));
    previous = current;
  }
...
export function fuzzyMatch(input: string, candidates: string[]): string | undefined {
  ...
  const maximumDistance = normalizedInput.length <= 4 ? 0 : normalizedInput.length <= 7 ? 1 : 2;
  let match: string | undefined;
  let minimumDistance = Infinity;
  let isAmbiguous = false;

  for (const candidate of candidates) {
    const distance = levenshteinDistance(normalizedInput, candidate.trim().toLowerCase());
...
function messageWords(message: string) { return message.match(/[a-z0-9]+/gi) ?? []; }
...
export function resolveInstructor(message: string) {
  ...
  const exactMatch = INSTRUCTORS.find((instructor) => instructor.split(" ").some((name) => hasWord(normalized, name)));
  ...
  for (const word of messageWords(normalized)) {
    ...
    if (matchedName) return INSTRUCTORS.find((instructor) => instructor.split(" ").includes(matchedName))!;
  }
  return undefined;
}
export function resolveClassType(message: string) {
  ...
  if (exactMatch) return exactMatch;
  for (const word of messageWords(message)) { const matchedType = fuzzyMatch(word, [...CLASS_TYPES]); if (matchedType) return matchedType as typeof CLASS_TYPES[number]; }
  return undefined;
}
export function resolveWeekday(message: string) {
  const exactMatch = WEEKDAYS.find((weekday) => hasWord(message, weekday));
  if (exactMatch) return exactMatch;
  for (const word of messageWords(message)) { const matchedWeekday = fuzzyMatch(word, [...WEEKDAYS]); if (matchedWeekday) return matchedWeekday as typeof WEEKDAYS[number]; }
  return undefined;
}
